...
%                                                              Forward pass
% -------------------------------------------------------------------------
%creat dispersed weight(binary,TWN,DoreFaNet)

BITW = 32;
BITA = 16;

function output=quantize(x,k)
    [a,b,c,d]=size(x.weights{1});
    output =x.weights{1};
    n_1 = 2^k - 1;
    for g=1:a
        for p=1:b
            for z=1:c
                for s=1:d
                 output(g,p,z,s)=round(x.weight{1}(g,p,z,s)*n_1)/n_1; 
                end
            end
        end
...
if(BITW==32) 
    output =x.weights{1};
elseif(BITW==1)
    E=sum(sum(sum(sum(x.weight{1}))))/(size(x.weight{1},1)*size(x.weight{1},2)*size(x.weight{1},3)*size(x.weight{1},4));
    for g=1:a
        for p=1:b
            for z=1:c
                for s=1:d
                 output(g,p,z,s)=round(x.weight{1}(g,p,z,s)/E)*E;
                 output1(g,p,z,s) = tanh(x.weight{1}(g,p,z,s));
                end
            end
        end
    ...
    MAX_weight = max(max(max(max(abs(output1)))));
    output = output1/MAX_weight*0.5+0.5;
end
end


function output = fa(x,BITA)
    output =x.weights{1};
if(BITA == 32)
    output =x.weights{1};
else
    output = quantize(x,BITA);
end
end


function output=binaried(x)
    [a,b,c,d]=size(x.weights{1});
    output =x.weights{1};
    for g=1:a
        for p=1:b
            for z=1:c
                for s=1:d
                    if(x.weights{1}(g,p,z,s)>=0)
                        output(g,p,z,s)=single(1);
%                         class(output(g,p,z,s))
                    else
                        output(g,p,z,s)=single(-1);
                    end
                end
...
for i=1:n
  ...
  switch l.type
    case 'conv'
%       Dis{i}.weights{1}=fw(l,BITW);
      Dis{i}.weights{1}=binaried(l);
      res(i+1).x = vl_nnconv(res(i).x, Dis{i}.weights{1}, l.weights{2}, ...
        'pad', l.pad, ...
        'stride', l.stride, ...
        'dilate', l.dilate, ...
        l.opts{:}, ...
    ...
    switch l.type
      ...
      case 'conv'
        [res(i).dzdx, dzdw{1}, dzdw{2}] = ...
          vl_nnconv(res(i).x, Dis{i}.weights{1}, l.weights{2}, res(i+1).dzdx, ...
          'pad', l.pad, ...
          'stride', l.stride, ...
          'dilate', l.dilate, ...
          l.opts{:}, ...
